...
def experiment_generative_kqm(config):
    ...
    compile_autoencoder_model(autoencoder, learning_rate*0.1)
    train_autoencoder_model(autoencoder, X_train, X_val, autoencoder_pretrain_epochs, autoencoder_pretrain_batch_size, autoencoder_pretrain_verbose,\
            wandb_enabled, autoencoder_shuffle, autoencoder_early_stopping_patience)

    compile_autoencoder_model(autoencoder, learning_rate*0.05)
    train_autoencoder_model(autoencoder, X_train, X_val, autoencoder_pretrain_epochs, autoencoder_pretrain_batch_size, autoencoder_pretrain_verbose,\
            wandb_enabled, autoencoder_shuffle, autoencoder_early_stopping_patience)
    ...
    input_model = tf.keras.layers.Input(shape=(32,32,3))

    encoder_full_sequential = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(512,)),
        tf.keras.layers.Flatten(),
        tf.keras.layers.Dense(512, activation=tf.nn.gelu),
        tf.keras.layers.Dropout(0.35),
        tf.keras.layers.Dense(encoded_size, activation=tf.nn.gelu),
        tf.keras.layers.Dropout(0.69)
    ])
    encoder_full = tf.keras.models.Model(input_model, encoder_full_sequential(encoder_kqm(input_model)))

    

    kqm_model = KQMModel(encoded_size=encoded_size, encoder=encoder_full, n_comp=n_comp)
    ...
    encoded_samples = initialize_prototypes_using_random_samples_from_the_training_set(kqm_model, X_train, y_train,\
            encoder_full, num_classes, n_comp, initial_prototype_modifier_multiplier)
    ...
    encoder_full.trainable = True
    for layer in encoder_full.layers:
    ...
    if wandb_enabled: 
        sigma = capture_sigma_parameter_and_update_it_on_wandb(sigma, kqm_model.kernel)
        config["sigma"] = sigma

    if visualization_enabled: 
        visualize_the_points_in_the_feature_space_and_plot_the_prototypes(kqm_unit, encoder_full, X_train, y_train)
    ...
    enc_x = encoder_full.predict(X_train)
    ...
    distribs = dm2distrib(kqmgen_model(tf.eye(10)), sigma=kqm_model2.kernel.sigma)
    ...
    for ii in range(10):
        ...
        for i in range(2):
            for j in range(5):
                ax[i][j].imshow(decoded_imgs[k], aspect='auto')
                k += 1
    ...
    if wandb_enabled: 
        wandb.config.update({"num_of_parameters_of_kqm_model" : num_of_parameters_of_kqm_model})
